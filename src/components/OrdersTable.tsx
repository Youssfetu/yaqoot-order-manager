          ...
          <div className="fixed inset-0 bg-black/50 flex items-center justify-center z-[100] p-4">
            <div className="bg-white rounded-lg shadow-xl max-w-md w-full p-6">
              <h3 className="text-lg font-semibold mb-4 text-right">
                تحرير التعليق - {selectedOrderForComment.code}
              </h3>
              
              {/* أزرار الأولوية */}
              <div className="mb-4">
                <p className="text-sm text-gray-600 mb-2 text-right">اختر الأولوية:</p>
                <div className="grid grid-cols-7 gap-2">
                  {[
                    { num: 1, color: "red", label: "عاجل جداً" },
                    { num: 2, color: "orange", label: "عاجل" },
                    { num: 3, color: "yellow", label: "مهم" },
                    { num: 4, color: "blue", label: "عادي" },
                    { num: 5, color: "green", label: "منخفض" },
                    { num: 6, color: "purple", label: "متأخر" },
                    { num: 7, color: "gray", label: "مؤجل" }
                  ].map((priority) => {
                    const isSelected = liveCommentText.startsWith(`${priority.num}. `);
                    return (
                      <button
                        key={priority.num}
                        onClick={(e) => {
                          console.log(`🔥 Modal Priority ${priority.num} button clicked!`);
                          e.preventDefault();
                          e.stopPropagation();
                          const priorityText = `${priority.num}. `;
                          const currentComment = liveCommentText || '';
                        ...
                        className={cn(
                          ...
                          "hover:scale-110 active:scale-95 border-2 focus:outline-none",
                          priority.color === "red" && (isSelected ? "bg-red-600 border-red-400 shadow-lg scale-105" : "bg-red-500 border-red-300 hover:bg-red-600"),
                          priority.color === "orange" && (isSelected ? "bg-orange-600 border-orange-400 shadow-lg scale-105" : "bg-orange-500 border-orange-300 hover:bg-orange-600"),
                          priority.color === "yellow" && (isSelected ? "bg-yellow-600 border-yellow-400 shadow-lg scale-105" : "bg-yellow-500 border-yellow-300 hover:bg-yellow-600"),
                          priority.color === "blue" && (isSelected ? "bg-blue-600 border-blue-400 shadow-lg scale-105" : "bg-blue-500 border-blue-300 hover:bg-blue-600"),
                          priority.color === "green" && (isSelected ? "bg-green-600 border-green-400 shadow-lg scale-105" : "bg-green-500 border-green-300 hover:bg-green-600"),
                          priority.color === "purple" && (isSelected ? "bg-purple-600 border-purple-400 shadow-lg scale-105" : "bg-purple-500 border-purple-300 hover:bg-purple-600"),
                          priority.color === "gray" && (isSelected ? "bg-gray-600 border-gray-400 shadow-lg scale-105" : "bg-gray-500 border-gray-300 hover:bg-gray-600")
                        )}
                        type="button"
                        title={priority.label}
                      >
              ...
              <textarea
                ...
                className="w-full h-32 p-3 border border-gray-300 rounded-lg resize-none focus:outline-none focus:ring-2 focus:ring-blue-500 text-right"
              ...
              <div className="flex gap-2 mt-4 justify-end">
                <button
                  onClick={() => {
                    console.log(`🔥 Saving comment: "${liveCommentText}" for order: ${selectedOrderForComment.id}`);
                    onUpdateComment(selectedOrderForComment.id, liveCommentText);
                    setSelectedOrderForComment(null);
                  }}
